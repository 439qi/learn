...
2. 浮点型  
    ...
    `NaN` 不满足自反性，因而 Rust 中浮点数是[偏序集合]，用于比较实现的是 `std::cmp::PartialEq` trait，不同于其他类型比较实现的 `std::cmp::Eq`  
    NFY: 偏序集合
...
- 范围模式(range pattern)  
    ...
    对于闭区间使用 `a..=b`；对于开区间使用 `a...b`，比 range 表达式多一个 `.`
...
将堆上内存赋给另一个变量时**移动**(move)，即浅拷贝后使原先变量无效  
