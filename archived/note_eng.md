# MyWords

| 单词                       |  发音                 | 词性                         | 含义                                                       |
| -------------------------- | ------------------- | ---------------------------- | ---------------------------------------------------------- |
| expository                 | []                  | adj.                         | 说明性的                                                   |
| token                      | [ˈtoʊkən]           | n.                           | 令牌（安全领域）；标记（词法分析）                         |
| notion                     | [ˈnoʊʃ(ə)n]         | n.                           | 概念，理念，观念                                           |
| delimit                    | [diˈlɪmɪt]          | n.                           | 界定；限定；定界限；                                       |
| parenthesis                | [pəˈrenθəsɪs]       | n.                           | 括弧；括号                                                 |
| punctuation                | [ˌpʌŋktʃuˈeɪʃ(ə)n]  | n.                           | 标点符号                                                   |
| denote                     | [dɪˈnoʊt]           | v.                           | 表示                                                       |
| compact                    | [kəmˈpækt]          | adj.                         | 紧凑的                                                     |
|                            |                     | n.                           | 契约                                                       |
| semantics                  | [səˈmæntɪks]        | n.                           | 语义，语意；语义学                                         |
| unconditional              | [ˌʌnkənˈdɪʃən(ə)l]  | adj.                         | 无条件的；无私的                                           |
| omit                       | [oʊˈmɪt]            | v.                           | 省略，忽略                                                 |
| glyph                      | [glɪf]              | n.                           | 石雕符号；象形文字；图像；字形；                           |
| metrics                    | ['metrɪks]          | n.                           | 指标；度量；font metrics: 字体规格，字体矩阵文件           |
| collate                    | [kəˈˌleɪt]          | v.                           | 整理；校对；定序，排序                                     |
| staple                     | [ˈsteɪp(ə)l]        | adj.                         | 基本的，主要的；标准的                                     |
|                            |                     | v.                           | 用订书钉钉住                                               |
| preempt                    | [priˈempt]          | v.                           | 抢占；取代                                                 |
| instantiate                | [ɪns'tænʃɪeɪt]      | v.                           | （instance 的动词形式）实例化                              |
| premature                  | [ˌpriməˈtjʊr]       | adj.                         | 过早的；不成熟的；仓促的                                   |
| duplex                     | [ˈduˌpleks]         | n.                           | 复式房屋；连体房屋；duplex printing: 双面打印              |
| manual                     | [ˈmænjuəl]          | n.                           | 说明书；指南；使用手册                                     |
|                            |                     | adj.                         | 手动的；手工的；体力的；manual feed 手动送纸               |
| convention                 | [kənˈvenʃ(ə)n]      | n.                           | 习俗；管理；协定；大会；会议；公约                         |
| conform                    | [kənˈfɔrm]          | v.                           | 顺从；遵从；随大流                                         |
| accommodate                | [əˈkɑməˌdeɪt]       | v.                           | 为... 提供住宿；为... 提供空间；为... 提供方便；(眼睛)聚焦 |
| tray                       | [treɪ]              | n.                           | 托盘；碟；浅塑料盒；input tray: 进纸盒                     |
| addendum                   | [əˈdendəm]          | n.                           | （尤指书籍的）补遗；补充；附录                             |
| proliferation              | [prəˌlɪfəˈreɪʃ(ə)n] | n.                           | 增殖；增生；（数量）激增                                   |
| intervention               | [ˌɪntərˈvenʃ(ə)n]   | n.                           | 干预；介入；调解                                           |
| canonical                  | [kəˈnɑnɪk(ə)l]      | adj.                         | 经典的；标准的；规范的                                     |
| contiguous                 | [kənˈtɪɡjuəs]       | adj.                         | 相接的；相邻的                                             |
| mandatory                  | [ˈmændəˌtɔri]       | adj.                         | 强制的；法定的；义务的                                     |
|                            |                     | n.                           | [法]受任者，代理人  |
| colon                      | []                  | n.                           | 冒号；结肠                                                 |
| qualifier                  | []                  | n.                           | 预选赛；资格赛；合格者；限定词；限定符；修饰符             |
| futile                     | []                  | adj.                         | 徒劳的；无效的；无益的；轻浮的                             |
| conceivably                | []                  | adv.                         | 可以想象地；令人信服地                                     |
| syntax                     | [ˈsɪnˌtæks]         | n.                           | 语法；语法规则；语法学                                     |
| ledger                     | [ˈledʒər]           | n.                           | 收支总账；分类账簿                                         |
| salient                    | []                  | adj.                         | 最重要的；显著的                                           |
|                            |                     | n.                           | 突出部分                                                   |
| supplementary              | []                  | adj.                         | 补充性的；额外的                                           |
|                            |                     | n.                           | 增补者                                                     |
| constitute                 | []                  | v.                           | 设立；组成，构成；被算作；任命；制定；建立                 |
| advantageous               | []                  | adj.                         | 有利的；有好处的                                           |
| arbitrary                  | []                  | adj.                         | 武断的，专横的；任意的                                     |
| subsequent                 | []                  | adj.                         | 随后的；后来的；附带的；并发的                             |
| subsidiary                 | []                  | adj.                         | 附带的；次要的；辅助的；                                   |
|                            |                     | n.                           | 子公司；附属公司                                           |
| reside                     | []                  | v.                           | 定居在，居住在；属于；驻留                                 |
| obsolete                   | []                  | adj.                         | 淘汰的；废弃的；过时的                                     |
|                            |                     | n.                           | 被废弃的事物                                               |
| isolated                   | []                  | adj.                         | 孤立的；分离的；偏僻的                                     |
| peculiar                   | [pɪˈkjuljər]        | adj.                         | 古怪的；奇异的；特有的；特殊的                             |
| entail                     | []                  | v.                           | 需要；牵涉；使必要                                         |
|                            |                     | n.                           | 限定继承权；继承人的顺序                                   |
| underlying                 | []                  | adj.                         | 根本的；潜在的；表面下的；基础的                           |
| dispose                    | []                  | v.                           | 布置；安排；处理；排列；dispose of: 处理，解决             |
| desinate                   | []                  | v.                           | 指定；选定；委派                                           |
| synthetically              | []                  | adv.                         | 综合地；人工合成地；                                       |
| straddle                   | []                  | v.                           | 横跨；跨坐；骑；分腿站立                                   |
|                            |                     | n.                           | 叉开双腿；大踏步                                           |
| elaboration                | []                  | n.                           | 认真做；精练；细化；加工；细致的工作；细致的作品；矫揉造作 |
| impose                     | []                  | v.                           | 迫使；强制实行；把... 强加于；推行；利用；欺骗             |
| preclude                   | []                  | v.                           | 排除；阻止；妨碍                                           |
| stencil                    | []                  | n.                           | 模板；模具                                                 |
| opaque                     | []                  | n.                           | 不透明；遮光涂料                                           |
|                            |                     | adj.                         | 模糊的；不透明的；浑浊的                                   |
| obscure                    | []                  | adj.                         | 朦胧的；模糊的；费解的；鲜为人知的                         |
|                            |                     | n.                           | 朦胧；黑夜                                                 |
|                            |                     | v.                           | 使模糊；使隐晦                                             |
| artifact                   | []                  | n.                           | 伪影；人工制品；工件                                       |
| polarity                   | []                  | n.                           | 截然对立；极性                                             |
| coincide                   | []                  | v.                           | 相符；一致；重叠；同时发生                                 |
| bulky                      | []                  | adj.                         | 庞大的；笨重的                                             |
| prolog                     | []                  | n.                           | 序言；开端；逻辑语言                                       |
| supersede                  | []                  | v.                           | 废除；代替；接替                                           |
| tailor                     | []                  | v.                           | 定制                                                       |
|                            |                     | n.                           | （专门定制衣服的）裁缝                                     |
| fidelity                   | [fɪˈdeləti]         | n.                           | 保真度；忠诚；忠实；忠贞；                                 |
| quota                      | [ˈkwoʊtə]           | n.                           | 配额；定额；指标；限额                                     |
| peripheral                 | [pəˈrɪf(ə)rəl]      | n.                           | 外部设备；附加设备；计算机外围设备                         |
|                            |                     | adj.                         | 周围的；外围的；外设的                                     |
| incorporate                | []                  | v.                           | 包含；吸收；使... 并入；将... 包括在内                     |
|                            |                     | adj.                         | 法定组织的；无形体的；精神上的；具象化了的                 |
| abbreviation               | [əˌbriviˈeɪʃ(ə)n]   | n.                           | 缩写；\[数学\]约分；\[音乐\]略号 |
| swift                      | []                  | adj.                         | 飞快的；迅速的                                             |
|                            |                     | n.                           | 雨燕                                                       |
| protrusion                 | [proʊˈtruʒ(ə)n]     | n.                           | 伸出；突出物                                               |
| auxiliary                  | [ɔɡˈzɪljəri]        | adj.                         | 辅助的；备用的                                             |
|                            |                     | n.                           | 助动词；辅助人员；辅助设备                                 |
| streak                     | [strik]             | n.                           | 条纹；条痕；（尤指不好的）性格特征                         |
|                            |                     | v.                           | 画条纹；使布满条纹；飞奔；疾驰                             |
| nozzle                     | [ˈnɑz(ə)l]          | n.                           | 喷嘴；管口                                                 |
| contone                    | []                  | n.                           | 连续色调                                                   |
| gamut                      | [ˈɡæmət]            | n.                           | 全部；全范围；色域；音阶                                   |
| obfuscate                  | [ˈɑbfəˌskeɪt]       | v.                           | 混淆；使模糊；使暗淡                                       |
| inkjet                     | []                  | adj.                         | 喷墨式                                                     |
| issuance                   | []                  | n.                           | 发行，颁布；配给，分配                                     |
| beforehand                 | []                  | adv.                         | 预先地；事先地；提前地                                     |
| simultaneous               | [ˌsaɪm(ə)lˈteɪniəs] | adj.                         | 同时发生的；并发的                                         |
| comprise                   | []                  | v.                           | 组成，构成；包括，包含；由... 组成                         |
| functionality              | []                  | n.                           | 功能；功能性；机能                                         |
| numeral                    | []                  | n.                           | 数字；数码                                                 |
|                            |                     | adj.                         | 数字的；数码的；表示数的                                   |
| interlace                  | []                  | v.                           | 使编结；交错；隔行扫描                                     |
| perceptual                 | []                  | adj.                         | 知觉的；感知的；感性的                                     |
| countermeasures            | []                  | n.                           | 对策；对抗手段；应对措施                                   |
| encompass                  | []                  | v.                           | 包含；包围；围绕                                           |
| entrust                    | []                  | v.                           | 委托；托付                                                 |
| acronym                    | [ˈækrəˌnɪm]         | n.                           | 缩写词；缩略语                                             |
| precaution                 | []                  | n.                           | 预防；预防措施；防备                                       |
|                            |                     | v.                           | 使提防；预先警告                                           |
| facilitate                 | []                  | v.                           | 促进；促使；使容易                                         |
| intuitive                  | [ɪnˈtuɪtɪv]         | adj.                         | 直觉的；直观的                                             |
| embellishment              | [ɪmˈbelɪʃmənt]      | n.                           | 装饰音；装饰品；修饰；润色；贪污；挪用                     |
| revise                     | [rɪˈvaɪz]           | v.                           | 修改；复习；改变                                           |
|                            |                     | n.                           | 修订；校正                                                 |
| merit                      | [ˈmerɪt]            | n.                           | 价值；优点；功绩；美德                                     |
|                            |                     | v.                           | 值得；应得                                                 |
| intangible                 | [ɪnˈtændʒəb(ə)l]    | adj.                         | 无实体的；模糊的                                           |
|                            |                     | n.                           | 不可捉摸的东西；无形；无形资产；无形的东西                 |
| integrated                 | [ˈɪntəˌɡreɪtəd]     | adj.                         | 集成的；完整的；综合的；各部分密切协调的                   |
| implicate                  | [ˈɪmplɪˌkeɪt]       | v.                           | 牵涉；表明...是起因                                        |
| consign                    | [kənˈsaɪn]          | v.                           | 交付；委托；寄存；把...置于(令人不快的境地)；打发          |
| propagate                  | [ˈprɑpəˌɡeɪt]       | v.                           | 传播；宣传；增殖，繁殖；传染；遗传                         |
| discrepancy                | [dɪˈskrepənsi]      | n.                           | 差异；不符合；不一致；矛盾                                 |
| entitle                    | [ɪnˈtaɪt(ə)l]       | v.                           | 使享有权利；使符合资格；授权                               |
| terminology                | [ˌtɜrmɪˈnɑlədʒi]    | v.                           | 术语；专业词汇                                             |
| sustenance                 | [ˈsʌstənəns]        | n.                           | 维持；营养；食物；养料                                     |
| allotmentə                 | [ˈlɑtmənt]          | n.                           | 分配；分配物；分配量；拨款                                 |
| criterion {plural}criteria || n.                  | 标准；准则；尺度；规范；判据 |
| interleaf                 | | n.                  | 夹层；夹入书页中的空白纸     |
|                            |                     | v.                           | 夹进；交错；交织                                           |
| colorant                   |                     | n.                           | 着色剂；颜料；染料                                         |
| glossary                   | [ˈɡlɔsəri]          | n.                           | 词汇表；术语表                                             |
| pertain   | [pərˈteɪn]    | v.    | 存在；适用；关于；属于    |
| preset    | [ˌpriˈset]    | v.    | 预设；预置；事先安排；给...预定时间   |
| precede   | [prɪˈsid]    | v.    | 先于；在...之前发生；走在..前面；领先；由于   |
| inherent  | [ɪnˈherənt]   | adj.  | 固有的；内在的；天生的；  |
| identical | [aɪˈdentɪk(ə)l]    | adj.  | 相同的；完全一样的    |
| aggregate     | [ˈæɡrɪˌɡeɪt]    | n.  | 合计；总数；  |
|   |   | v.    | 合计；总计    |
|   |   | adj.  | 总计的；总数的    |
| affinity  | [əˈfɪnəti]    | n.    | 喜好；爱好；密切的关系    |
| mutual    | [ˈmjutʃuəl]   | adj.  | 相互的；彼此的；共有的    |
| proprietary   | [prəˈpraɪəˌteri]  | n.    | 所有权；所有物；所有团体；    |
|   |   |adj.   | 专卖的；专营的；专利的；所有的；专属的；    |
| indentation   | [ˌɪndenˈteɪʃ(ə)n] | n.| 行首缩进；凹陷；缺口  |
| stash | [stæʃ]    | v.    | 隐藏；贮藏；存放  |
|   |   | n.    | 贮藏物    |
| repository    | [rɪˈpɑzəˌtɔri]    | n.    | 仓库；贮藏室；存放处；学识渊博的人    |
| margin    | [ˈmɑrdʒɪn]    | n.    | 边缘；差额；余地；界限；外边距；保证金    |  
|   |   |v. | 给...镶边；给...加旁注；为...付保证金   |
| benchmark | [ˈbentʃˌmɑrk] | n.    | 基准；水准点；标准检查程序    |
||| v.   | 用基准问题测试   |
| assimilate    | [əˈsɪmɪˌleɪt] | v.    |同化；吸收；使相似；将...比作...；彻底了解   |
| lingual   | ['lɪŋgwəl]    | adj.  | 舌头的；话语的；语言的；舌音的    |  
|   |   | n.    | 舌神经；舌动脉；舌音；舌音字母    |
| materialization   | [məˌtɪərɪrlaɪ'zeɪʃn]  | n.    | 实体化；具象化；赋予形体  |
 | dangle    |  [ˈdæŋɡ(ə)l]  | v.    | 悬挂；悬空；吊着  |
 |consecutive|   [kənˈsekjətɪv] | adj.  | 连续不断的；连贯的    |
 | slant    |  [slænt]  | n.    | 倾斜；斜面；倾斜度；斜线；    |
 ||| v. |倾斜；有倾向性地陈述；使倾斜 |
 ||| adj.   |倾斜的 |
| alphanumeric  | [ˌælfənuˈmerɪk]   | adj.  | 含有字母和数字的；字母和数字并用的    |
| evict     | [ɪˈvɪkt]  | v.    | 驱逐；逐出；收回  |
| isomorphic    | [ˌaɪsə'mɔrfɪk]    | adj.  | 同构的；同型的；  |
| monotonic     | [mɒnəˈtɒnɪk]  | adj.  | 单调的；没有变化的    |
| modulo    | ['mɒdjəˌloʊ]  | prep. | 取模  |
| corridor  | [ˈkɔrɪˌdɔr]   | n.    | 通道；过道；国境线走廊    |
| permutation   | [ˌpɜrmjʊˈteɪʃ(ə)n]    | n.    | 置换；排列（方式）；组合（方式）|
| adjacent  | [əˈdʒeɪs(ə)nt]    | adj.  | 邻近的|
| vowel | [ˈvaʊəl]  | n.    | 元音；元音字母    |
|   | | adj.    | 元音的    |
| permute   | [pə'mjuːt]    | v.    | 交换，取代；置换；[数]排列    |
| placard   | [ˈplæˌkɑrd]   | n.    | 海报；标语牌；广告牌  |
||| v.   | 在...贴公告；在...贴广告；（用布告等）公布   |
| proclaim  | [prəˈkleɪm]   | v.    | 宣告；宣布；表明；声明    |
| ensue | [ɪnˈsu]   | v.    | 接着发生；因而产生    |
| subside   | [səbˈsaɪd]    | v.    | 沉降；消退；平息  |
| directive | [daɪˈrektɪv]  | n.    |指示；命令  |
||| adj.    | 指示的；指导的|
| conformance   | [kən'fɔrməns] | n.    | 一致性；遵从；恪守    |
| pun   | [pʌn] | n.    | 双关；双关语  |
|||v.| 使用双关|
| hitherto  | [ˈhɪðərˌtu]   | adv.  | 迄今为止，至今  |
| niche | [nɪtʃ]    | n.    | 市场定位；商机；舒适或称心的工作、生活；生态位；壁龛；（山体）凹进的地方  |
| sanction  | [ˈsæŋkʃ(ə)n]  |  n.   | 批准；制裁；处罚  |
||| v.  | 许可；准许；惩罚  |
| nominal   | [ˈnɑmɪn(ə)l]  | adj.  | 名义上的；很小的；象征性的；名词性的，名词的    |
| evangelist    | [ɪˈvændʒəlɪst]    | n.    | 布道者；传教士    |
| multitude | [ˈmʌltɪˌtud]  | n.    | 人群，群众；众多，大量    |
| antitrust | [ˌæntiˈtrʌst]    | adj.  | 反垄断的，反托拉斯的  |
| legislation   | [ˌledʒɪˈsleɪʃ(ə)n]    | n.    | 立法；法规；法律  |
| misnomer  | [mɪsˈnoʊmər]  | n.    | 用词不当；误用词；误称    |
| predates  | [ˌpriˈdeɪt]   | v.    | （在日期上）早于，先于    |
| hoard | [hɔrd]    | v.    | 囤积；贮藏；（尤指）秘藏  |
|||n.   | （钱、食物、贵重物品等的）贮存；（尤指）秘藏  |
| acronym   | [ˈækrəˌnɪm]   | n.    | 首字母缩写词      |
| militate  | [ˈmɪlɪˌteɪt]  | v.  | 产生作用或影响；妨碍；战斗    |
| embodiment    | [ɪmˈbɑdɪmənt]    | n.   | 体现；具象化；化身   |
| baptized  | [bæpˈtaɪz]    | v.  | 施洗；洗礼；受洗  |
| unidirectional    | | adj.    | 单向的；单向性的  |
| bidirectional | [ˌbaɪdaɪˈrekʃ(ə)nəl]   | adj.  | 双向的，双向性的    |
| unsolicited   | [ˌʌnsəˈlɪsɪtəd]   | adj.  | 自发的；未经请求的；未经同意的    |  
| semaphore | [ˈseməˌfɔr]   | adj.  | 信号量；信号标；旗语  |
||| n.  | 发信号；打旗语    |
| nifty | [ˈnɪfti]    | adj.  | 精确的；有技巧的；实用的；漂亮的|
||| n.  | 俏皮话；漂亮的东西  |
| auspex    | ['ɔ:spɪsɪz]   | n.    | auspex复数：赞助；主办；（古罗马）鸟兽占卜者  |
| preemptive    | [priˈemptɪv]    | adj.  | 抢占式的；[政]先发制人的    |
| hierarchy | [ˈhaɪəˌrɑrki] | n.    | 层次体系；阶层；等级制度；统治集团    |
| poll  | [poʊl]  | n.   | 轮询 |
| intervening   | [ˌɪntərˈvinɪŋ]    | adj.  | 介于中间的    |
| agile     | [ˈæˌdʒəl] | adj.  | 灵活的；敏捷的；机智的    |
| cyclically    |   | adv.| 周期地  |
| synonymous    | [sɪˈnɑnɪməs]  | adj.  | 同义的    |
| pseudo     |  ['sjuːdəʊ]  | adj.    | 假的；虚拟的；冒充的    |
| miscellaneous | [ˌmɪsəˈleɪniəs]   | adj.  | 混杂的；各种各样的    |
|   failover    | [ˈfɛlˌoʊvə]   | n.    | [IT]故障切换；故障转移能力    |
| hostile   | [ˈhɑst(ə)l]    | adj.  | 敌意的；敌对的；坚决否定的    |
||| n.  | 敌对分子  |
| catastrophic  | [ˌkætəˈstrɑfɪk]   | adj.  | 灾难性的；悲惨的  |
| commence  | [kəˈmens] | v.    | 开始，开始发生；着手开始做  |
| temporal  | [ˈtemp(ə)rəl] | adj.  | 时间的；世俗的；现世的    |
|||n.   | 太阳穴；世俗的权力；一时的事物    |
| upshot    |  [ˈʌpˌʃɑt]    | n.    | 最后结果；结局；结论  |
| monolithic    |  [ˌmɒnəˈliθik]    | adj.  | 庞大的；整体式的；单一的  |
| concatenation     | [kənˌkætəˈneɪʃ(ə)n]   | adj.  | 串联；一系列相关联的事物  |
| Impersonate   | [ɪmˈpɜrs(ə)nˌeɪt] | v.    | 冒充；扮演；假扮  |
| scalability   | [skeɪlə'bɪlɪtɪ]   | n.    | 可扩展性；可伸缩性    |
| bolster   |[ˈboʊlstər]| v.    |改善，加强    |
| asterisk  | [ˈæstərɪsk]   | n.    | 星号  |
|   ||v.    | 标注星号  |
| multiplex | [ˈmʌltɪˌpleks]    | adj.  | 多样的；多路复用的    |
|||n.   | 多路；多厅影院    |
|||v.   | 多路传输  |
| fidelity  | [fɪˈdeləti]   | n.    | 保真度；忠诚；忠贞    |
| deferral  |   | n.    | 延期；延迟    |
| eligible  | [ˈelɪdʒəb(ə)l]    | adj.  | 有资格的；合格的；|
|||n.   | 合适的人  |
| departure| [dɪˈpɑrtʃər]   | n.    | 离开；出发；背离  |
| depict    | [dɪˈpɪkt] | v.    | 描述；描绘；刻画  |
| versatile | [ˈvɜrsət(ə)l] | adj.  | 多用途的；通用的；多才多艺的  |
| envision  |  [ɪnˈvɪʒ(ə)n] | v.    | 预想；展望；想象  |
| interoperable | [ˌɪntər'ɒpərəbl]  | adj.  | 客户想操作的；互通的  |
| ubiquitous    | [juˈbɪkwɪtəs] | adj.  | 无处不在的；普遍存在的    |
| anatomy   | [əˈnætəmi]    | n.    | 解剖；解析；剖析  |
| nudges    | [nʌdʒ]    | n.    | 推动；(温和的)劝说    |
|||v.   | 轻推；逐步推进    |
|editorial  | [ˌedəˈtɔriəl] | adj.  | 编者的；主编的；  |
|||v.   | 社论；期刊的评论；    |
| fallacy   | [ˈfæləsi] | n.    | 谬误；谬论    |
| anagram   | [ˈænəˌɡræm]   | n.    | 颠倒字母而成的词句；字谜游戏  |
| nest| [nest]  | n.    | 嵌套；巢  |
| lexicographically | | adv.    | 按词典顺序    |
| citation     | [saɪˈteɪʃ(ə)n]    | n.    | 引证；引述    |
| ergonomics    | [ɜrɡəˈnɑmɪks]  | n.    | 人体工程学；工效学(研究如何改善工作条件以提升工作效率)    |
| precedence    | [ˈpresɪdəns]  | n.    | 优先级；优先权    |
| rationale | [ˌræʃəˈnæl]   | n.    | 基本原理；根本原因；理论基础|
| deliberate    | [dɪˈlɪb(ə)rət]    | n.    | 深思熟虑；仔细思考|
|||adj. | 故意的；不慌不忙的；深思熟虑的|
|defective |[dɪˈfektɪv]|adj.   | 有缺点的；有缺陷的；|
|||n.   | 次品；有缺陷的人/物|
| malicious | [məˈlɪʃəs]| adj.  | 怀有恶意的；恶毒的；蓄意的|
| deem  | [dim] | v.    | 认为；视作；知道  |
| interoperability  | ['ɪntərɒpərə'bɪlətɪ]| n.  | 互用性|
| bootstrap |  ['buːtstræp] | adj.  | 自举的；依靠自己力量的；  |
||| n.  | 自举；拔靴带；  |
| marshal   | [ˈmɑrʃ(ə)l]   | v.    | [计]封送处理；收集；安排；控制人群 ；维持秩序   |
||| n.  | (英国的)空军元帅，陆军元帅；司仪；(一些美国城市的)警察局长，消防局长  |
| proportionate    | [prəˈpɔrʃ(ə)nət]   | adj.  | 成比例的；相应的；相称的  |
|||v.   | 使成比例；使适应；|
| traverse | [trəˈvɜrs] | v.    | 穿过；横渡；|
|||n.   | 可横越的地方|
|ephemeral  | [ɪˈfem(ə)rəl] | adj.  | 短暂的；瞬息的；|
|cabinet    | [ˈkæbɪnət]    | n.  | 内阁；储藏柜；陈列柜|
|||n.| 内阁的；小房间用的，玲珑的；细木工做的   |
| plunder   | [ˈplʌndər]    | n.    | 掠夺；掠夺的财物  |
|||v.   | 掠夺  |
| deliberately  | [dɪˈlɪb(ə)rətli]  | adv.  | 有意地，故意地；从容地，不慌不忙地    |
| impasse   | [ˈɪmˌpæs] | n.    | 僵局；绝境    |
| elide     | [ɪˈlaɪd]  | v.    | 削减；不考虑；省略    |
| feasible  | [ˈfizəb(ə)l] | adj.   | 可行的    |
| orthogonal    | [ɔr'θɒgənəl]  | adj.  | 正交的；直角的；垂直的；（统计学）不相关的    |
| superfluous   | [sʊˈpɜrfluəs] | adj.  | 过剩的；多余的；不必要的  |
| glucose   |  [ˈɡluˌkoʊz]  | n.    | 葡萄糖    |
| contiguity    | [ˌkɒntɪ'ɡjuətɪ]   | n.    | 连续性；接近  |
| appertain | [ˌæpərˈteɪn]  | v.    | 属于；与...有关   |
| conduit   | | n.  | 导管|
| plumb | [plʌm]    | n.    | 垂直；铅锤|
|||v.   | 探索；钻研|
||| adv.    | 恰恰；正好；完全，彻底    |
||| adj.    | 垂直的；公正的；完全的    |
| plumbing |[ˈplʌmɪŋ] | n.    | 管路系统；水暖工的工作  |
||| n.  | plumb 现在分词    |
|circumvent | [ˈsɜrkəmˌvent]    | v.    | 规避，设法回避；绕过；绕行    |
| lengthy   | [ˈleŋθi] |adj. | 冗长的；漫长的；很长的|
| coordination  | [koʊˌɔrdɪˈneɪʃ(ə)n]   | n.    | 协调；配合；协作的能力    |
| allude    |[əˈlud]    | v.    | 暗指；(委婉地)提到    |
| refine    | [rɪˈfaɪn] | v.    | 精炼；改进；改善；提纯；  |
| notation |  [noʊˈteɪʃ(ə)n]    | n.    | 记谱法；标记法；  |
| snippet   | [ˈsnɪpət] | n.    | (消息,知识等)的片断；零星的东西；小片断 |
| stumble   | [ˈstʌmb(ə)l]  | v.    | 绊倒；跌跌撞撞地走；踌躇；说不出话来    |
|||n.   | 绊倒；过失；失败  |
|hypothetical |[ˌhaɪpəˈθetɪk(ə)l]   | adj.  | 假设的；假定的；  |
| pedantic  |[pəˈdæntɪk]    | adj.  | 迂腐的；学究气的|
| axiom | [ˈæksiəm] | n.    | 公理；定理    |
| designate | [ˈdezɪɡnət]   | v.    | 指定；指派|
|||adj. | (已受委派)尚未上任；(已当选)尚未就职  |
| locus | [ˈloʊkəs] | n.    | (某事发生的)确切地点；(被视为某物的)中心  |
|pamphlet   |[ˈpæmflət] | n.    | 小册子，手册|
|memorandum | [ˌmeməˈrændəm]    | n.    | 便笺；协议备忘录；|
| racket    | [ˈrækɪt]  | n.    |球拍；吵闹；骗局，敲诈|
|||v.   |用球拍打；吵闹|
|muffler    | [ˈmʌflər] | n.    | 围巾；(发动机的)消音器|
| orchestra |[ˈɔrkəstrə]    |n. |管弦乐队；管弦乐；(舞台前的)正厅前排 |
| puncture  | [ˈpʌŋktʃər]   | n.    | 刺伤；(轮胎上被刺破的)小孔；(皮肤上被刺破的)扎孔|
|||v.   | 在...上穿孔，扎孔；挫伤(锐气等)|
|volitional | [və'lɪʃ(ə)nəl]    | adj.  | 意志的|
|imperative| [ɪmˈperətɪv]   | adj.  | 紧急的；迫切的；权威的|
|||n.   |重要紧急的事；必要的事；祈使语气   |
|brooch | [broʊtʃ]    | n.    | 胸针  |
| verbose   | [vɜrˈboʊs]    | adj.  | 冗长的；啰嗦的    |
|trunk| [trʌŋk] | n.    | 树干；躯干；后备箱|
|chime  | [tʃaɪm]   |v. |敲响；鸣响；报时|
|||n.   |钟声；响亮清晰的声音；铃声|
| truism|  [ˈtruˌɪzəm]|n.   | 真理；不言而喻的道理；老生常谈|
| factoid| [ˈfækˌtɔɪd]| n.  |(看似真实但实际上并不完全准确的信息或小知识)；伪事实|
|baffle| [ˈbæf(ə)l]|v.|使困惑，难住|
|||n.|挡板|
|sanity |[ˈsænəti] |n.|理智，心智；心智健全|
|dogma|[ˈdɔɡmə]|n.| 教条；信条；教义|
