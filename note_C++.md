...
- 命名空间 namespace：用于封装名字，防止污染全局命名空间。 名字空间不改变声明的意义，仅改变可见性
- 限定修饰名字符 qualified name notation  `std::string`
...
|     |                                                                   |
| :-- | :---------------------------------------------------------------- |
| L/l | long 类型，缺省默认为 int 型。在字符类型前时表示为宽字符，wchar_t |
| U/u | 无符号数。不可用于浮点型字面常量                                  |
| E/e | 科学计数法                                                        |
| F/f | 单精度浮点数，缺省默认为 double 型。不可用于整型字面常量          |
...
声明编译时可以对函数或变量求值  
constexpr变量和给定了实参的constexpr函数可以用于需要编译期常量表达式的地方  
...
- 不能出现在函数体中
- 链接指示符可以出现在同一文件中的多个声明中。若只出现在函数的第一次声明中，后续声明都接受第一个声明中链接指示符指定的链接规则
...
- 优先查找使用该名字的域，若没有则查找父级域直至全局域。仍没有则报错
- 这个过程依赖于该名字是如何被使用的 以及使用该名字的域
...
- 局部对象local object：局部域中声明的对象。三种局部对象由其所在存储区的属性和生命期区分
  ...
  | 寄存器对象 <br>register object       | 寄存器     | /                     | 仅是建议，部分编译器可能忽略                                |
...
> 函数模板提供一个种用来自动生成各种类型函数实例的算法。程序员对于函数接口（参数和返回类型）中的全部或者部分类型进行参数化parameterize而函数体保持不变
...
- 显式模板实参：显式指定explicitly specify模板实参
  `Derived<int> a;`
...
在 C++的异常处理机制是不可恢复的 nonresumptive:一旦异常被处理,程序的执行就不能够在异常被抛出的地方继续

#### try

可以包含任何 C++语句——表达式以及声明。一个 try 块引入一个局部域，try 块内声明的变量不能在 try 块外被引用
...
- 如果throw表达式位于try块中则检查与try块相关联的catch子句，若没有则在主调函数中继续查找。该过程沿着嵌套函数调用链向上继续，直到找到该异常的catch子句
...
1. 选择候选函数除9.3重载解析三种外4. 在左操作数的类中被声明的成员运算符集合5. C++内置运算符集合
2. 选择可行函数
3. 选择最佳匹配函数
...
4. `std::enable_shared_from_this`  
    ...
    基于[SFINAE](#sfinaesubstitution-failure-is-not-an-error)
5. `.reset()` 、`.reset(new T)`  
    调用`.reset()`时会释放资源  
...
在考虑成员的访问级别之前对它进行名字解析 基本的出发点是要防止程序语义的微妙改动，而这种语义一般与成员访问级别无关。即名字的实际指向不会因修改成员访问级别而改变
...
1. 选择候选函数
   ...
   - 成员函数在派生类中的成员函数声明并没有重载基类中声明的同名成员函数。相反 派生类中的成员函数隐藏了基类中同名成员函数的声明 即使函数参数表并不相同
...
- 指针输出指针所持有的地址值 `const char*`被解释为c风格字符串，输出为字符串。可以强转为void*输出地址值
...
5. 锁的底层原理分为软件实现和硬件实现，软件实现需要通过一些特别的算法**not finished**且开销比硬件实现更高
   ...
   - 锁的本质  
     在计算机里本质上是一块内存空间，可以是flag，1表示以上锁，0表示未上锁。关键在于如何保证只有一个线程能竞争到锁，即上锁的操作必须是原子操作，无法被其他操作打断
   - 原子操作  
    原子操作需要操作系统支持
    一般通过CPU的原子指令来实现原子操作
